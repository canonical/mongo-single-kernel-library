...
class ApplicationCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(self.on.write_releases_action, self._on_write_releases)
    ...
    def _on_write_releases(self, event: ActionEvent):
        first_database_relation = self.first_database.relations[0]
        connection_string = self.first_database.fetch_relation_field(
            first_database_relation.id, field="uris"
        )
        client = MongoClient(
            connection_string,
        ...
        db = client[event.params["database"]]

        test_collection = db["test_collection"]

        # Clean collection writes from previous test.
    ...
    def _on_create_user(self, event: ActionEvent):
        first_database_relation = self.first_database.relations[0]
        connection_string = self.first_database.fetch_relation_field(
            first_database_relation.id, field="uris"
        )
        ...
        client.admin.command(
            ...
            pwd=event.params["password"],
            roles=[{"role": "readWrite", "db": event.params["database"]}],
        )
        client["new_database"]
        client.close()
