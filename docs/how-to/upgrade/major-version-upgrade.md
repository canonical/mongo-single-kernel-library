...
{ref}`Log into your MongoDB 6 cluster <access-a-replica-set>` and create the MongoDB 8 internal users (`charmed-operator`, `charmed-backup`, `charmed-logrotate`, `charmed-stats`).
...
Replicate all the passwords from your initial application to the new one. Create a new Juju secret including `operator`, `backup`, `logrotate` and `monitor` usernames and theirs passwords and set it to the `system-user` configuration option of your orchestrator application as it is described [here](https://canonical-charmed-mongodb.readthedocs-hosted.com/8/how-to/manage-passwords/).
...
Replicate all the passwords from your initial application to the new one. Create a new Juju secret including `charmed-operator`, `charmed-backup`, `charmed-logrotate` and `charmed-stats` usernames and theirs passwords and set it to the `system-user` configuration option of your orchestrator application as it is described [here](https://canonical-charmed-mongodb.readthedocs-hosted.com/8/how-to/manage-passwords/).
