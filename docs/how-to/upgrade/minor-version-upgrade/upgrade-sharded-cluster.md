...
```{warning}
Do not proceed if this step was not successful. If your refresh failed, {ref}`roll back <verify-the-success-of-the-refresh>` the cluster.
```
...
```{warning} 
Do not proceed if an upgrade of a shard fails. If an upgrade fails, {ref}`roll back <verify-the-success-of-the-refresh>` the entire cluster.
```
